...
For easier configuration, you can use the simplified `llm` array to configure multiple LLM endpoints. Each LLM requires a CA certificate for secure connections.
...
- `name` - Display name for this LLM configuration
- `endpoint` - API endpoint URL
- `apiKey` - API key for authentication
- `model` - Model name
- `caCert` - Path to CA certificate file (required for all connections)

:::note
The `caCert` path supports `~` expansion to the user's home directory.
:::

The `llm` configuration takes precedence over traditional `provider` configuration. Models configured via `llm` will appear in the `opencode models` command output with provider IDs prefixed with `llm-`.
